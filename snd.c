...
SOUND::SOUND(const char filedir[], MATRIX_f *m, VECTOR *pos, float max_distance)
{
    total=0;
    dir=filedir;
    camera_m=m;
...
void SOUND::play(const char filename[], VECTOR *pos)
{
    int i;
    for(i=0;i<total;i++)
...
void SOUND::play(const char filename[], float pos[3])
{
    int i;
    for(i=0;i<total;i++)
    if(!strcmp(filename, name[i]))
...
void SOUND::play(const char filename[], unsigned char volume, unsigned char pan, int speed, bool loop)
{
    int i;
    for(i=0;i<total;i++)
    if(!strcmp(filename, name[i]))
...
void SOUND::adjust(int sound_nr, unsigned char volume, unsigned char pan, int speed, bool loop)
{
    adjust_sample(sample[sound_nr], volume, pan, speed, loop);
}

void SOUND::adjust(int i, VECTOR *pos, int volume)
{
       float dist;
    dist=vector_length_f(camera_pos->x - pos->x, camera_pos->y - pos->y, camera_pos->z - pos->z);
   MATRIX_f cam_inv;
   invert_matrix(camera_m, &cam_inv);
    if (dist <= maxdist)
    {
       ...
       if(vol>224)vol=224;
       get_sound_heading(*camera_m, pos, &heading);
       adjust_sample (sample[i], volume, 128-128*heading.x, 1000, 1);
    }
}

void SOUND::adjust(const char filename[],  VECTOR *pos, int volume)
{
    int i;
    for(i=0;i<total;i++)
    if(!strcmp(filename, name[i]))
