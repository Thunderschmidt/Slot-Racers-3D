...
void SOUND_CLASS::stop(const char filename[])
{
    int i;
    for(i=0;i<total;i++)
    if(!strcmp(filename, name[i]))
...
void SOUND_CLASS::adjust(int i, VECTOR *pos, int volume)
{
       float dist;
    dist=vector_length_f(camera_pos->x - pos->x, camera_pos->y - pos->y, camera_pos->z - pos->z);
   MATRIX_f cam_inv;
...
void SOUND_CLASS::adjust(const char filename[],  VECTOR *pos, int volume)
{
    int i;
    for(i=0;i<total;i++)
    if(!strcmp(filename, name[i]))
