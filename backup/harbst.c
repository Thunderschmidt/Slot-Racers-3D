...
void stop_a_action(ACTION *act) //löscht alle aktiven actions, die so sind wie act
{
   if(act->active)
   {
   a_action_anz--;
...
ACTION* get_action(MODEL3D_CLASS *model3d)//Funktion findet die letzte Action einer Verknüpfungsreihe
{
   int i, j;
   ACTION *link;
   for(i=0; i<a_action_anz; i++)
   if(((a_action[i]->type == ACT_WAIT) || (a_action[i]->type == ACT_WAITFOR)) && (a_action[i]->model3d == model3d)&&(a_action[i]->action_group==model3d->get_action_group()))  //es gibt immer nur höchstens eine AKTIVE WAIT(_FOR)-Action. Ist eine aktiv?
   {
      link=a_action[i];
...
void activate_action(ACTION *act)//Diese Funktion aktiviert inaktive Actions (rekursiv, alles zwischen Wait-Befehlen)
{
   int i;
   ACTION link;
   act->model3d->clear_a_action(act->o, act->axis, act->type); //könnte probleme machen!!
...
void MODEL3D_CLASS::kill_all_actions() //löscht alle actions eines MODEL3D-Objektes
{
   int i;
   for(i=0;i<a_action_anz;i++)
   if(a_action[i]->model3d==this)
