...
void MODEL3D::clear_action_group(int a_g)
{
   ...
   int i, t;
   for(i=0;i<a_action_total;i++)
   if((a_action[i]->model3d==this) && (a_action[i]->action_group==a_g)){kill_a_action(i);i--;}
   for(i=0;i<i_action_total;i++)
   if((i_action[i]->model3d==this)&&(i_action[i]->action_group==a_g)){ kill_i_action(i);i--; printf("%d", a_g);}
}
...
//                 pl=blinking->col16[blinking->is_on];
 ...
 //              printf("frame_nr:%d action_nr: %d action: %d\n", frame_nr, i, a_action[i]);
         ...
         case ACT_WAIT:
              ...
              {
                 a_action[i]->type=ACT_FINISHED;
                 if(a_action[i]->link != ACT_NONE){ printf(" activate:"); activate_action(a_action[i]->link);}
